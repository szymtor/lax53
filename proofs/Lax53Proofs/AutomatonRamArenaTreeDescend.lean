...
namespace Lax53Proofs.AutomatonRamArenaTreeDescend
...
theorem descendTreeNode_spec (B : Nat) (I : WordImage)
  ...
  obtain ⟨childAddress, restAddress, hcursorTag, hchildWord, hrestWord,
      hchildRep, hrestRep⟩ :=
    Lax53Proofs.ArenaSemantics.Represents.fields_cons hpre.2.2.2
  have hcursorValid :=
    Lax53Proofs.ArenaSemantics.Represents.valid hpre.2.2.2
  have hchildValid :=
    Lax53Proofs.ArenaSemantics.Represents.valid hchildRep
  have hrestValid :=
    Lax53Proofs.ArenaSemantics.Represents.valid hrestRep
  have hcursorLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hcursorValid
  have hchildLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hchildValid
  have hrestLen :=
  ...
  have hgetB (i : Nat) : (arenaWords I).getD i 0 < B :=
    Lax53Proofs.AutomatonRamCorrectness.getD_lt_of_mem_bound
      (by omega) hvaluesB
  ...
  have hcore : Spec B (fun tau => tau = sigma) descendTreeNode
    ...
    unfold descendTreeNode Lax53Proofs.AutomatonRamProgram.seqs
    run_vcg [hpush]
  ...
  have hWlength := Lax53Proofs.Run.arrayLength_eq hrun "W"
  have hSymbolLength :=
    Lax53Proofs.Run.arrayLength_eq hrun "TreeSymbolStack"
  have hTailLength :=
    Lax53Proofs.Run.arrayLength_eq hrun "TreeTailStack"
...
end Lax53Proofs.AutomatonRamArenaTreeDescend
