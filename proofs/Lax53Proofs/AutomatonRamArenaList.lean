...
private theorem cursorLookup (I : WordImage) (address tag : Nat)
  ...
  have hlt := Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hvalid
...
theorem representedListCondition_value {alpha : Type} (B : Nat)
    ...
    (representedListCondition cursorVariable).evalB B sigma =
      some (!xs.isEmpty) := by
  have hvalid := Lax53Proofs.ArenaSemantics.Represents.valid hrep
  have hcursorLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hvalid
  ...
  have htag := Lax53Proofs.ArenaSemantics.Represents.list_tag hrep
  cases xs with
  | nil =>
