...
theorem evaluator_spec (B : Nat) (M : EncodedAutomaton)
  ...
  have hinput : automatonInput M t =
      parameter.length :: (parameter ++ (word.length :: word)) := by
    simp [automatonInput, modelCheckingInput, block, parameter, word]
  ...
  have hpPre : (sigma.arrs "P").length = parameter.length ∧
    ...
    exact ⟨hsigma.1, hinput ▸ hsigma.2.2.2.2⟩
  obtain ⟨sigma1, hrun1,
  ...
  have hevalPre : EvaluateTreeContext M word sigma2 := by
    rcases hheader with ⟨hP2, hInp2, hA2, hQ2, hT2, hR2, hwidth2,
      hrecords2, hacceptBase2, hF2, hn2⟩
    exact ⟨⟨hP2, hA2, hQ2, hT2, hR2, hwidth2, hrecords2,
      hacceptBase2, hF2, hn2⟩, hInp2, hS2, hL2, hO2⟩
  obtain ⟨sigma3, hrun3,
  ...
  have hrun : Run B evaluator sigma sigma4 (evaluatorImpCost M t) := by
    have hrunExact :=
      hrun1.seq (hrun2.seq (hrun3.seq (hrun4.seq Run.skip)))
    have hacceptCost := Nat.mul_le_mul_left
      (39 * M.2.2.2.length + 34) hrootLenLeT
    change Run B
      (.seq readParameter (.seq readHeader
  ...
  change sigma4.out = sigma.out ++
    [if (M.2.toAutomaton M.1).Accepts t then 1 else 0]
  rw [hscanPost, hevalOut (by decide), hhOut (by decide), hpOut (by decide)]
  rw [haccept]
