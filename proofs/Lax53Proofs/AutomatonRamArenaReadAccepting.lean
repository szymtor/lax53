...
theorem acceptingBody_spec (B : Nat) (I : WordImage) (base : Nat)
  ...
  | cons acceptState rest =>
      ...
      have hvalueValid :=
        Lax53Proofs.ArenaSemantics.Represents.valid hvalueRep
      have hrestValid :=
      ...
      unfold acceptingBody Lax53Proofs.AutomatonRamProgram.seqs
      run_vcg
...
theorem readAccepting_spec (B : Nat) (I : WordImage)
  ...
  have hspaceLoop : (encodeAutomaton (alphabet, body)).length ≤
    ...
    rw [Lax53Proofs.Run.arrayLength_eq hrunLoop "P"]
    exact hspace
  ...
  refine ⟨final, ?_, ?_⟩
  · unfold readAccepting Lax53Proofs.AutomatonRamProgram.seqs
