...
namespace Lax53Proofs.IntrinsicFormulaInput
...
open Lax53Proofs.AutomatonRamArenaCorrectness Lax53Proofs.FormulaArenaTraversalOrder
open Lax53Proofs.PrimitiveRecursivePairing Lax53Proofs.CompilerArrayPacking
open Encodable
...
theorem program_ok (L : Lax13Proofs.Compile.Layout) (ht : 9 ≤ L.temps)
    (hs : ∀ s ∈ scalars, s ∈ L.scalars) (ha : ∀ a ∈ arrays, a ∈ L.arrays) :
    Lax13Proofs.Compile.Com.Ok L program := by
  ...
  have hread : Lax13Proofs.Compile.Com.Ok L IntrinsicOccurrenceInput.program := by
    refine ⟨?_, hpacked⟩
    simp only [MSORamCompilerProgram.loadCompilerOccurrence, AutomatonRamProgram.seqs,
      Lax13Proofs.Compile.Com.Ok, Lax13Proofs.Compile.Expr.Ok]
    ...
    omega
  simp only [program, loop, step, condition, Lax13Proofs.Compile.Com.Ok, hread,
    pairProgram, Lax13Proofs.Compile.Cond.Ok, Lax13Proofs.Compile.Expr.Ok,
    Lax13Proofs.Compile.condExpr]
  rcases hscalar with ⟨hcf, hfo, hso, htag, hfc, hrc, hci, hcnt, hname, hfw, hsw, hms, hA⟩
  simp_all only [true_and, and_true]
  omega

end Lax53Proofs.IntrinsicFormulaInput
