...
namespace Lax53Proofs.EmptyMarkers
...
theorem emptyMark_mem_formulaLanguage_iff {A : RankedAlphabet.{u}}
    (t : Tree A) (phi : Lax52.MSOSyntax.Sentence (treeSignature A)) :
  ...
  have hsem : MarkedRealize (emptyMark t) phi emptyNode emptySet ↔ TreeModels t phi := by
    ...
    have hVmap : (fun X => e '' emptySet X) = targetSet := by
      funext X
      exact Fin.elim0 X
    have h := (Lax53Proofs.MSOSemantics.realize_equiv e phi emptyNode emptySet).symm
