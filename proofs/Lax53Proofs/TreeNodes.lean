...
namespace Lax53Proofs.TreeNodes

open Lax53.RankedTree
open Lax53.RankedTree.Node
...
theorem childAt_childNode {A : RankedAlphabet.{u}} {t : Tree A}
    ...
    Lax53.RankedTree.Node.ChildAt (ChildIndex.ofSymbolIndex p.label i) p
      (childNode p i) := by
  exact ⟨i, rfl, rfl⟩
...
theorem childAt_functional {A : RankedAlphabet.{u}} {t : Tree A}
    ...
    (hq : Lax53.RankedTree.Node.ChildAt i p q)
    (hr : Lax53.RankedTree.Node.ChildAt i p r) : q = r := by
...
theorem childAt_iff_eq_childNode {A : RankedAlphabet.{u}} {t : Tree A}
    ...
    Lax53.RankedTree.Node.ChildAt (ChildIndex.ofSymbolIndex p.label i) p q ↔
      q = childNode p i := by
  constructor
  · intro h
