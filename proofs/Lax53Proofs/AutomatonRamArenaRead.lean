...
theorem readArena_spec (B : Nat) (raw : Raw)
  ...
  have hrootLast : I.root + 3 = I.memoryWords := by
    exact Lax58Proofs.WordArena.encodeRaw_root_last raw
  ...
  have hread := (Lax53Proofs.ArrayInput.readArr_spec B
...
structure InstancePointers (I : WordImage) (M : EncodedAutomaton)
  ...
  automatonRep : I.Represents automaton
    (Lax53.StructuralRepresentations.automatonPresentation.toRaw M)
...
theorem instancePointers (M : EncodedAutomaton)
  ...
  have hroot : I.Represents I.root (automatonTreeRaw M t) := by
    exact Lax58.WordArena.encodeRaw_represents _
  ...
  obtain ⟨alphabetAddress, bodyAddress, hautomatonTag, halphabetWord,
    ...
    Lax53Proofs.ArenaSemantics.Represents.prod_fields hautomatonRep
  exact ⟨{
    ...
    automatonValid := Lax53Proofs.ArenaSemantics.Represents.valid hautomatonRep
...
theorem openInstanceAt_spec (B : Nat) (I : WordImage)
  ...
  have hautomatonLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length p.automatonValid
  have htreeTailLen :=
  ...
  have hbodyLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length
      (Lax53Proofs.ArenaSemantics.Represents.valid p.bodyRep)
  have hfieldsGet : (arenaWords I)[I.root + 2] = p.fields := by
    rw [← Lax53Proofs.ArrayInput.getD_eq_getElem hrootLen]
    exact p.fieldsWord
  have hautomatonGet : (arenaWords I)[p.fields + 1] = p.automaton := by
    rw [← Lax53Proofs.ArrayInput.getD_eq_getElem (by omega)]
    exact p.automatonWord
  have htreeTailGet : (arenaWords I)[p.fields + 2] = p.treeTail := by
    rw [← Lax53Proofs.ArrayInput.getD_eq_getElem hfieldsLen]
  ...
  have hbodyGet : (arenaWords I)[p.automaton + 2] = p.body := by
    rw [← Lax53Proofs.ArrayInput.getD_eq_getElem hautomatonLen]
    exact p.bodyWord
