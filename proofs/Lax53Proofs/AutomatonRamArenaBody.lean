...
theorem openAutomatonBody_spec (B : Nat) (I : WordImage)
  ...
  obtain ⟨qAddress, tailAddress, hrootTag, hqWord, htailWord,
      hqRep, htailRep⟩ :=
    Lax53Proofs.ArenaSemantics.Represents.prod_fields hbodyRep'
  ...
  have hqPayload :=
    Lax53Proofs.ArenaSemantics.Represents.nat_payload hqRep
  have hrootValid := Lax53Proofs.ArenaSemantics.Represents.valid hbodyRep
  have hqValid := Lax53Proofs.ArenaSemantics.Represents.valid hqRep
  ...
  have hacceptValid := Lax53Proofs.ArenaSemantics.Represents.valid hacceptRep
  have hrootLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hrootValid
  have hqLen :=
    Lax53Proofs.ArenaSemantics.ValidAddress.lt_arenaWords_length hqValid
  have htailLen :=
  ...
  have hacceptGet : (arenaWords I)[tailAddress + 2] = acceptAddress := by
    rw [← Lax53Proofs.ArrayInput.getD_eq_getElem htailLen]
    exact hacceptWord
