...
namespace Lax53Proofs.PrimitiveRecursiveRam

open Lax13.Ram Lax13.RamComputes Lax13Proofs.Imp Lax13Proofs.Reasoning
...
def program (c : Code) : Program := Lax13Proofs.Compile.compileProgram (layout c) (runner c)
...
theorem runner_ok (c : Code) : Lax13Proofs.Compile.Com.Ok (layout c) (runner c) := by
  have hzero : reg 0 ∈ (layout c).scalars := by
    apply List.mem_map.mpr
    exact ⟨0, List.mem_range.mpr (by have := space_ge_three c; omega), rfl⟩
  have hc := layout_ok c
  simpa [runner, Lax13Proofs.Compile.Com.Ok, Lax13Proofs.Compile.Expr.Ok, layout] using
    (show reg 0 ∈ (layout c).scalars ∧
      Lax13Proofs.Compile.Com.Ok (layout c) (compile c 0) ∧
      reg 0 ∈ (layout c).scalars from ⟨hzero, hc, hzero⟩)

theorem runner_solves (c : Code) (n : Nat) :
    Lax13Proofs.Transfer.Solves (layout c) (runner c) {[n]}
...
theorem code_computes (c : Code) (n w : Nat) (hfit : wordBound c n ≤ 2 ^ w) :
  ...
  apply Lax13Proofs.Transfer.computesInTime_of_solves (runner_solves c n)
  · intro _ _
    refine ⟨by omega, ?_, ?_⟩
...
end Lax53Proofs.PrimitiveRecursiveRam
