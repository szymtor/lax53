...
namespace Lax53Proofs.IntrinsicCompilerPrepare
...
open Lax53Proofs.FormulaArenaTraversalModel Lax53Proofs.FormulaArenaTraversalInit
open Lax53Proofs.FormulaArenaTraversalInvariant Lax53Proofs.FormulaArenaTraversalRead
...
def Initial (B : Nat) (alphabet : RankedAlphabetCode)
  ...
  (σ.arrs "Arena").length = (encodeRaw (msoTreeRaw alphabet φ t)).memoryWords ∧
    ...
    σ.inp = Lax53.MSOLinearTime.modelCheckingInput alphabet φ t
...
theorem program_spec (B : Nat) (alphabet : RankedAlphabetCode)
    ...
    (hwidth : Lax53.TreeModelCheckingEncoding.maximumRank alphabet + 3 < B)
  ...
  obtain ⟨σ1, hread, hloaded⟩ := readArena_spec B (msoTreeRaw alphabet φ t) hmem hinputB
    σ ⟨hArena, by simpa [Lax53.MSOLinearTime.modelCheckingInput] using hinput⟩
  ...
  have hP2 : (σ2.arrs "P").length = (σ.arrs "P").length :=
    Lax53Proofs.Run.arrayLength_eq (hread.seq hopen) "P"
  ...
  have hcapacity3 : Capacity B (postorder alphabet φ).length σ3 := by
    intro a ha
    simpa [Lax53Proofs.Run.arrayLength_eq hprefixRun a] using hcapacity a ha
...
end Lax53Proofs.IntrinsicCompilerPrepare
