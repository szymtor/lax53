...
open Lax560851.WordArena Lax842588Proofs.IntrinsicCompiledTableBounds
...
theorem time_le (M : EncodedAutomaton) (t : Tree M.1.toRankedAlphabet) :
  ...
  have hm := Lax560851Proofs.WordArena.encodeRaw_memoryWords_proof (treeStructure M.1 t)
...
theorem exists_fixed_automaton (M : EncodedAutomaton) :
  ...
  have hmem : (encodeRaw (treeStructure M.1 t)).memoryWords < B := by
    rw [Lax560851Proofs.WordArena.encodeRaw_memoryWords_proof]
  ...
  have hvalues : ∀ v ∈ (encodeRaw (treeStructure M.1 t)).toInput, v < B :=
    Lax560851Proofs.WordArena.encodeRaw_toInput_lt _ B hpayload (by
      rw [Lax560851Proofs.WordArena.encodeRaw_memoryWords_proof] at hmem
