...
def valueBound (c d : Code) (alphabet : RankedAlphabetCode)
  ...
  wordCore c d (parameterSize alphabet φ) *
    (Lax842588.MSOLinearTime.inputStructuralSize alphabet φ t +
      Lax842588.MSOLinearTime.inputPayloadMax alphabet φ t + 1)
...
theorem resources (c d : Code) (alphabet : RankedAlphabetCode)
  ...
  let S := Lax842588.MSOLinearTime.inputStructuralSize alphabet φ t
  let N := S + Lax842588.MSOLinearTime.inputPayloadMax alphabet φ t + 1
  ...
  have hpayload : Lax842588.MSOLinearTime.inputPayloadMax alphabet φ t <
      valueBound c d alphabet φ t := by
    have hN : N ≤ k * N := Nat.le_mul_of_pos_left _ (by omega)
    exact (show Lax842588.MSOLinearTime.inputPayloadMax alphabet φ t < N by
...
end Lax842588Proofs.IntrinsicWordBounds
