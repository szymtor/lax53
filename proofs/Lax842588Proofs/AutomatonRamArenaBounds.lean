...
namespace Lax842588Proofs.AutomatonRamArenaBounds
...
theorem treeStructure_nodes_add_one (alphabet : RankedAlphabetCode)
  ...
  | node symbol children ih =>
      ...
      have hsymbolRaw :
          ((Lax560851.CertifiedDerivation.CertifiedFieldEncoding.fin alphabet.length).toRaw
            symbol).nodes = 1 := rfl
...
theorem inputStructuralSize_le (M : EncodedAutomaton)
  ...
  have htreePositive := Lax842588Proofs.AutomatonRamCorrectness.treeSize_pos M.1 t
...
theorem prepareCost_le (M : EncodedAutomaton)
  ...
  have hF := (acceptingCount_le_automatonSize M).trans
    (automatonSize_le_automatonWorkSize M)
  have hn := Lax842588Proofs.AutomatonRamCorrectness.treeSize_pos M.1 t
  have hmemory :
      (Lax560851.WordArena.encodeRaw (automatonTreeRaw M t)).memoryWords =
        3 * inputStructuralSize M t := by
    simpa [inputStructuralSize] using
      Lax560851Proofs.WordArena.encodeRaw_memoryWords_proof
...
theorem arenaInput_values_lt_valueBound (M : EncodedAutomaton)
  ...
  · have hmemory := Nat.le_of_lt (arenaMemory_lt_valueBound M t)
    rw [Lax560851Proofs.WordArena.encodeRaw_memoryWords_proof] at hmemory
...
theorem arenaLayout_fitsWords (M : EncodedAutomaton)
    ...
    Lax842588Proofs.AutomatonRamArenaProgram.layout.FitsWords
  ...
  · apply max_le
    ...
    · simp [Layout.span, Lax842588Proofs.AutomatonRamArenaProgram.layout,
        Lax842588Proofs.AutomatonRamProgram.layout, evaluatorValueBound,
        uniformWordBound]
      have hpositive : 0 < (automatonWorkSize M + 1) ^ 2 *
          (inputStructuralSize M t + inputPayloadMax M t + 1) :=
        (resourceBase_bounds M t).2.2.2
...
end Lax842588Proofs.AutomatonRamArenaBounds
