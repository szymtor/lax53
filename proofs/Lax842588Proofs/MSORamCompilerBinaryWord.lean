...
theorem beginBinaryWord_spec (B length state maximumRank : Nat) :
  ...
  have hdivisorB : 2 ^ (length - 1) < B := by
    exact lt_of_le_of_lt (Nat.pow_le_pow_right (by omega) (Nat.sub_le length 1))
      hpowerB
  unfold beginBinaryWord Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
...
theorem binaryWordBody_spec (B length state maximumRank : Nat) :
  ...
  have hdivisorNext : index + 1 < length →
    ...
    omega
  unfold binaryWordBody Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
...
theorem binaryPaddingBody_spec (B length state maximumRank : Nat) :
  ...
  unfold binaryPaddingBody Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
...
private theorem beginBinaryRows_scalars_spec (B length : Nat) :
    Spec B
      ...
      20 := by
  intro sigma hready
  unfold beginBinaryRows Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
  all_goals simp_all
...
end Lax842588Proofs.MSORamCompilerBinaryWord
