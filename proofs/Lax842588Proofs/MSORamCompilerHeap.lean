...
namespace Lax842588Proofs.MSORamCompilerHeap
...
theorem initializeCompiler_spec (B maximumRank : Nat) (h0 : 0 < B) :
  ...
  intro sigma _
  unfold initializeCompiler Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
...
theorem appendTransitionWord_spec (B maximumRank : Nat)
  ...
  have hwords' := wordsAt_set_append (value := value) hwords (by
    simpa [hcursor] using hslot)
  unfold appendTransitionWord Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
...
theorem appendAcceptingWord_spec (B maximumRank : Nat)
  ...
  have hwords' := wordsAt_set_append (value := value) hwords (by
    simpa [hcursor] using hslot)
  unfold appendAcceptingWord Lax842588Proofs.AutomatonRamProgram.seqs
  run_vcg
  all_goals simp_all [AcceptingHeapRep, appendAcceptingEnv]

end Lax842588Proofs.MSORamCompilerHeap
