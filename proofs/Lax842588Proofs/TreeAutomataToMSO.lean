...
namespace Lax842588Proofs.TreeAutomataToMSO
...
theorem isRoot_iff_eq_root (t : Tree A) (p : Node t) :
  ...
  constructor
  · intro hp
    rcases Lax842588Proofs.TreeNodes.root_or_exists_parent p with hroot | ⟨_, parent, hparent⟩
    · exact hroot
  ...
  · intro p
    ...
    have hchildStates : (fun i => stateLabel V hpart (Node.child p i)) = qs := by
      ...
      have hmem := hchildren i (Node.child p i)
        (Lax842588Proofs.TreeNodes.childAt_childNode p i)
...
theorem labeling_to_runValuation (M : Automaton A Q) (t : Tree A)
  ...
  · intro p ai hlabel
    ...
    have witness :
      ...
      · intro i y hy
        have hy' := (Lax842588Proofs.TreeNodes.childAt_iff_eq_childNode p y i).mp hy
        subst y
        simp [V, qs]
    rw [← hlabel]
    exact witness
...
/--
---
conclusion: Lax842588.MSOTreeAutomataEquivalence.automaton_definable_by_mso
---
-/
...
end Lax842588Proofs.TreeAutomataToMSO
