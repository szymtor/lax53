...
namespace Lax842588Proofs.ArenaSemantics
...
theorem Represents.nat_payload {I : WordImage} {address payload : Nat}
    (h : I.Represents address (Lax560851.StructuralCombinators.nat.toRaw payload)) :
    (arenaWords I).getD (address + 1) 0 = payload :=
  (Lax842588Proofs.ArenaSemantics.Represents.nat_words h).2.1
...
theorem Represents.fields_nil {I : WordImage} {address : Nat}
  ...
  exact (Lax842588Proofs.ArenaSemantics.Represents.nat_words h).1
...
theorem Represents.fields_cons {I : WordImage} {address : Nat}
  ...
  simpa [Raw.fields] using
    (Lax842588Proofs.ArenaSemantics.Represents.pair_words h)

theorem Represents.list_nil {α : Type} {P : Presentation α}
    {I : WordImage} {address : Nat}
    (h : I.Represents address ((Lax560851.StructuralCombinators.list P).toRaw [])) :
    (arenaWords I).getD address 0 = WordImage.natTag := by
  exact (Lax842588Proofs.ArenaSemantics.Represents.nat_words h).1

theorem Represents.list_cons {α : Type} {P : Presentation α}
    {I : WordImage} {address : Nat} {x : α} {xs : List α}
    (h : I.Represents address ((Lax560851.StructuralCombinators.list P).toRaw (x :: xs))) :
    ∃ xAddress restAddress,
      (arenaWords I).getD address 0 = WordImage.pairTag ∧
      (arenaWords I).getD (address + 1) 0 = xAddress ∧
      (arenaWords I).getD (address + 2) 0 = restAddress ∧
      I.Represents xAddress (P.toRaw x) ∧
      I.Represents restAddress ((Lax560851.StructuralCombinators.list P).toRaw xs) := by
  simpa [Lax560851.StructuralCombinators.list, listToRaw] using
    (Lax842588Proofs.ArenaSemantics.Represents.pair_words h)
...
theorem Represents.list_tag {α : Type} {P : Presentation α}
    ...
    (h : I.Represents address ((Lax560851.StructuralCombinators.list P).toRaw xs)) :
    (arenaWords I).getD address 0 = if xs.isEmpty then
      WordImage.natTag else WordImage.pairTag := by
  cases xs with
  | nil =>
      simpa using Lax842588Proofs.ArenaSemantics.Represents.list_nil h
  | cons x xs =>
      obtain ⟨_, _, htag, -⟩ :=
        Lax842588Proofs.ArenaSemantics.Represents.list_cons h
      simpa using htag

theorem Represents.prod_fields {α : Type} {β : Type} {A : Presentation α}
    {B : Presentation β} {I : WordImage} {address : Nat} {x : α} {y : β}
    (h : I.Represents address ((Lax560851.StructuralCombinators.prod A B).toRaw (x, y))) :
    ∃ xAddress yAddress,
      (arenaWords I).getD address 0 = WordImage.pairTag ∧
      (arenaWords I).getD (address + 1) 0 = xAddress ∧
      (arenaWords I).getD (address + 2) 0 = yAddress ∧
      I.Represents xAddress (A.toRaw x) ∧ I.Represents yAddress (B.toRaw y) := by
  simpa [Lax560851.StructuralCombinators.prod] using
    (Lax842588Proofs.ArenaSemantics.Represents.pair_words h)

end Lax842588Proofs.ArenaSemantics
