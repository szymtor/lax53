...
namespace Lax842588Proofs.MSOSemantics
...
@[simp] theorem realize_imp {n m : Nat} {phi psi : Formula L n m}
    {x : Fin n → M} {V : Fin m → Set M} :
    Realize (Lax146103.MSOSyntax.Formula.imp phi psi) x V ↔
...
@[simp] theorem realize_allFO {n m : Nat} {phi : Formula L (n + 1) m}
    {x : Fin n → M} {V : Fin m → Set M} :
    Realize (Lax146103.MSOSyntax.Formula.allFO phi) x V ↔
...
end Lax842588Proofs.MSOSemantics
