...
theorem program_spec (B : Nat) (M : EncodedAutomaton) (t : Tree M.1.toRankedAlphabet)
  ...
  obtain ⟨υ, hread, ha⟩ := readArena_spec B (treeStructure M.1 t) hmem hvalues τ
    ⟨by rw [Lax842588Proofs.Run.arrayLength_eq hload "Arena"]; exact hA,
      (hload.frame_inp (FixedTableLoader.program_noRead _)).trans hi⟩
  have hrB : υ.vars "root" < B := by
    rw [ha.2.1]
    have := Lax560851Proofs.WordArena.encodeRaw_root_last (treeStructure M.1 t)
    omega
  ...
  · have hrep := Lax560851Proofs.WordArena.encodeRaw_represents_proof (treeStructure M.1 t)
...
end Lax842588Proofs.FixedSentencePrepare
