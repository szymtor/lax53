...
`Lax560851.WordArena.encode` construction. Any evaluator-specific representation
engineering belongs only to the proof package and is charged there.
-/

namespace Lax842588.StructuralRepresentations
...
noncomputable def sentencePresentation (alphabet : RankedAlphabetCode) :
    Presentation (Lax146103.MSOSyntax.Sentence
      (treeSignature alphabet.toRankedAlphabet)) :=
  Lax560851.StructuralPresentation.presentationOf (formulaStructure alphabet)
...
end Lax842588.StructuralRepresentations
