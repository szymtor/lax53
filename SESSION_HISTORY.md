...
## 2026-09-16 — Archived status before final registration

The following status is historical. The subsequent final-registration entry
supersedes its keep-draft and unfinished-publication restrictions.

<details>
<summary>Earlier CURRENT_STATE.md</summary>
...
## 2026-09-16 — Final publication and registration completed

After reviewing the local preview, the user explicitly requested submission,
GPT author normalization, and final registration. They confirmed the scope is
the six current Lean 4.33 versions; older drafts are excluded. They also
confirmed that no renderer bug report should be posted. This authorization
supersedes the earlier keep-draft decision and the repository's registration
restriction for this release.

The tree release is `7790103f255ce90a858c51d10ad858b4b230f8b9`, with author
credit `GPT 5.6`. It includes the reviewed nine-claim interface. All 173 Lean
source files exactly match the preview's source hashes. The dependencies pin
registered word-MSO `a850aa03d9cb3bff8aec3990269c99942ac56aff`, canonical
encodings `2748fdc0afc0b4f729b7775ca6475ba08129418a`, RAM/Turing
`134e398cef46599371df99f10c9007bc34ba3868`, and Lax808846
`9394e531cc51cb67a0214bca3f9264dfe97ba5c7`.

Full exact-pin local `lax build . --replay --no-color` passed in 66m55s:
concepts 28s, proofs 6m32s, independent kernel replay 58m55s, inspection 58s.
It inspected ten concepts and nine annotated proofs. The recorded warnings
include deliberately retained helpers/proof dependencies and the canonical
draft state in the snapshot taken before its registration completed. All
dependencies were verified registered before tree submission/registration.

Archive validation workflow 35085094609 passed independently. The CLI lost
contact with GitHub near completion; `lax submit --resume` reattached to that
same successful run without another submission. Fresh local and accepted
Archive `inputs`, dependency lists, concept records, and all nine proof records
match exactly. All nine claims have closed proofs through the registered
dependency network. Registration workflow 35091939469 then succeeded.

The final six-record audit checked registered states, frozen source commits,
capture provenance, GPT author names, exact concept source text, proof counts,
complete proof closure, and all direct registered dependency pins. All 48
public claims across the six submissions are proven. Logs and release ledger
are under `../migration-tools/finalize-*` and `finalization-state.json`.

Both manifests are now generated from accepted exact pins; ordinary Lake
commands need no development override. Later documentation commits do not
change the immutable registered release. No proof-validation or publication
work remains. The renderer workaround remains local; it was not submitted
as mathematical source and no developer issue was posted.
