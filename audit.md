...
- `../canonical-encodings/concepts/Lax560851/StructuralDerivation.lean`, especially
  `FieldEncoding`;
- `concepts/Lax842588/StructuralRepresentations.lean`, especially `tree_lawful`
  and `sentence_lawful`.

### 2. There are definite Lax validation blockers

- `proofs/Lax842588Proofs/RuntimeLayout.lean` declares names under
  `Lax842588.TreeModelCheckingEncoding`, although proof-package declarations must
  live under `Lax842588Proofs`.
- The proof root does not directly import `Lax842588Proofs.RuntimeLayout`; Lax
  requires exact direct root imports.
- `proofs/RadixScratch.lean` is scratch material inside the proof package but
  outside `Lax842588Proofs/`; it must be removed or relocated before validation.
- The proof package directly requires `Lax560851Proofs`, but not the `Lax560851`
  ...
  using the assumption-free theorems from `Lax560851Proofs`, or add a direct
  concept dependency.

Relevant locations:

- `proofs/Lax842588Proofs/RuntimeLayout.lean`;
- `proofs/Lax842588Proofs.lean`;
- `proofs/RadixScratch.lean`;
- `proofs/lakefile.toml`;
- `proofs/Lax842588Proofs/MSORamArenaRead.lean`;
- `proofs/Lax842588Proofs/AutomatonRamArenaBounds.lean`;
- `proofs/Lax842588Proofs/StructuralRepresentations.lean`;
- `proofs/Lax842588Proofs/MSOLinearTime.lean`.
...
- `Lax842588.TreeModelCheckingEncoding.automatonInput_length`, apparently a small
  oversight;
- `Lax842588.MSOLinearTime.exists_uniform_msoModelChecking`;
- `Lax842588.MSOLinearTime.exists_fixed_sentence_modelChecking`.
